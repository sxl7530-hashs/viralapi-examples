...
1. 确认 `VIRALAPI_API_KEY` 只在服务端环境变量中，前端、Git 和普通日志都没有密钥。
2. 用 `curl --max-time 20` 发一个带 `X-Request-ID` 的 smoke test，先确认 `/v1` base URL 和模型名。
3. 分开统计 401/403、429、5xx、客户端超时、fallback 成功和最终失败，不要只看总成功率。
4. 按 `tenant_id` 检查并发、队列长度、P95、降级率、token 用量和成本分组占比。
5. 连续失败达到阈值后短时熔断；批量任务进队列和死信队列，客服链路返回明确的降级提示。
