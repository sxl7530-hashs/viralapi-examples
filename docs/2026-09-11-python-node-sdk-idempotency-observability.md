...
1. 只在服务端读取 `VIRALAPI_API_KEY`，确认密钥未进入前端、Git 和普通日志。
2. 用 `curl --max-time 20` 发 smoke test，携带业务生成的 `X-Request-ID`，确认 `/v1` base URL 和模型名。
3. 统计 401/403、400、429、5xx、客户端超时、fallback 成功、重复任务命中和最终失败。
4. 按 `tenant_id` 检查请求量、P95、token 用量、成本分组、队列长度和降级率。
5. 连续失败触发短时熔断；批量作业进入重试队列和死信队列，客服链路返回可解释的降级提示。
6. 用唯一索引保护 `(tenant_id, job_id)`，发布前做一次超时重放测试，确认不会产生重复业务结果。
