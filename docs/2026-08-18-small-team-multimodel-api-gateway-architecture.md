---
title: "小团队多模型 API 网关架构：Claude/GPT/Gemini 统一调用、tenant 路由与上线清单"
description: "面向小团队的 ViralAPI 多模型 API 网关生产架构：OpenAI-compatible 统一调用 Claude、GPT、Gemini，覆盖 tenant 路由、fallback、超时重试、熔断、成本分组和日志字段。"
date: 2026-08-18
---
...
3. 所有请求写入 `request_id`、`tenant_id`、`feature`、`model`、`cost_group`、`latency_ms`、`attempt`、`degraded`、`final_status`。
